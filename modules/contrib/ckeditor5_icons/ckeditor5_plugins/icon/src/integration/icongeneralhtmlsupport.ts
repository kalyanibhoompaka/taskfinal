...
import { setViewAttributes, type GHSViewAttributes } from '@ckeditor/ckeditor5-html-support/src/utils';
...
export default class IconGeneralHtmlSupport extends Plugin implements PluginInterface {
  ...
  init() {
    ...
    conversion.for('editingDowncast').add(dispatcher => {
      dispatcher.on<DowncastAttributeEvent>('attribute:linkHref:icon',
        (_evt, data, conversionApi) => {
          if (!conversionApi.consumable.consume(data.item, 'attribute:htmlLinkAttributes:icon')) {
            return;
          }
          const modelElement = data.item as ModelElement;
          ...
          if (viewElement) {
            setViewAttributes(conversionApi.writer, modelElement.getAttribute('htmlLinkAttributes') as GHSViewAttributes, viewElement);
          }
        },
        { priority: 'low' }
      );
    ...
    conversion.for('dataDowncast').add(dispatcher => {
      dispatcher.on<DowncastAttributeEvent>('attribute:linkHref:icon',
        (_evt, data, conversionApi) => {
          ...
          const viewContainerElement = viewElement?.parent;
          if (viewContainerElement?.is('element', 'a')) {
            setViewAttributes(conversionApi.writer, modelElement.getAttribute('htmlLinkAttributes') as GHSViewAttributes, viewContainerElement);
          }
        },
        { priority: 'low' }
      );
      dispatcher.on<DowncastAttributeEvent>('attribute:htmlAttributes:icon',
